    ...
    courses: [
      {
        id: 1,
        name: '.Net',
        description: 'Curso de .Net pra crear Api restful documentada con Swagger',
        level: 1,
        shortDescription: 'Curso de .Net'
      }
    ]
  },
  {
  ...
  public filterData(firstName?:string, lastName?: string, birthday?: Date): Student[]{
    return this.data.filter((student: Student, index:number) => {
      if(firstName && !lastName){
        ...
        return student.firstName.includes(firstName) && student.lastName.includes(lastName);
      }
      return student
    });
  }
